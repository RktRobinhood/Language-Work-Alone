...
In TOK terms, we must ask: If our language lacks a word for a concept, does the concept truly exist for the speaker? This leads to the distinction between 'Strong Determinism' (language as a cage) and 'Weak Relativity' (language as a lens). While translation proves we can move between concepts, the effort required to translate a 'native' concept suggests that some knowledge is inherently tied to its linguistic vessel.`,
  ...
  [StationId.TRANSLATION]: {
    ...
    reading: "Translation is not a mechanical mapping of word A to word B. It is an act of negotiation. Consider the Greek word 'Logos'—it can mean word, reason, proportion, or divine plan. When translated into Chinese as 'Dào', it inherits the connotations of 'The Path' and 'Flow'. The translator must choose which facet of meaning to preserve, often at the expense of others. This is 'Semantic Leakage.' In religious and legal texts, a single 'mistranslation' can change the course of history. TOK asks: Is 'perfect translation' an epistemological impossibility?",
    x: 45, y: 75,
  ...
  [StationId.ACQUISITION]: {
    ...
    reading: "Is language a biological instinct (Chomsky) or a learned behavior (Skinner)? Chomsky's Universal Grammar suggests we are born with a blueprint for language. Skinner argued language is learned through reinforcement. TOK asks: If language is innate, does that mean all humans share a basic logical framework? Or does the diversity of languages prove that our logic is purely cultural?",
    x: 10, y: 55,
  ...
  [StationId.AI_MT]: {
    ...
    reading: "AI models are trained on human-produced text, which is laden with historical biases. When translating from gender-neutral languages (like Turkish) to gendered ones (like English), AI often defaults to stereotypes—translating 'O bir doktor' as 'He is a doctor' and 'O bir hemşire' as 'She is a nurse'. The AI does not 'know' the world; it knows the statistical probability of words. This raises a TOK concern: Is AI amplifying human prejudice under the guise of 'objective' data?",
    x: 85, y: 65,
  ...
  [StationId.JARGON]: {
    ...
    difficultyDC: 12,
    reading: "Jargon allows for precision within a community of knowers, but it serves as a gatekeeper to those outside. Medical, legal, and academic jargon can obscure simple truths, creating an asymmetry of power. If you cannot speak the language of the expert, do you have access to their knowledge? TOK asks: Is jargon necessary for precision, or is it a tool for maintaining elite status?",
    x: 92, y: 40,
  ...
  [StationId.HUMOR]: {
    ...
    difficultyDC: 10,
    reading: "Humor often relies on phonetic accidents or cultural 'insider' knowledge. A pun in one language is rarely funny in another. Jokes reveal the boundaries of what a culture considers sacred or ridiculous. TOK asks: Is humor the most difficult form of knowledge to share between cultures?",
    x: 65, y: 85,
